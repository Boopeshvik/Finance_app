...
def get_current_score(
    ...
    monthly_plan = (
        ...
        .filter(MonthlyPlan.user_id == uid, MonthlyPlan.month == month, MonthlyPlan.year == year)
        .first()
    )
    ...
    if monthly_plan:
        ...
        plan_adherence_score = max(0, 100 - (income_diff_pct + expense_diff_pct + savings_diff_pct) / 3)
    else:
    ...
    if budgets:
        ...
        for budget in budgets:
            category_expense = sum(t.amount for t in transactions if t.type == "expense" and t.category.lower() == budget.category.lower())
            total_budget       += budget.amount
            total_budget_spent += category_expense
            score = 100 if category_expense <= budget.amount else max(0, 100 - ((category_expense - budget.amount) / budget.amount) * 100) if budget.amount > 0 else 0
            category_scores.append(score)
            budget_category_results.append({"category": budget.category, "budget": round(budget.amount, 2), "spent": round(category_expense, 2), "score": round(score, 2)})
        budget_adherence_score = sum(category_scores) / len(category_scores)
    else:
    ...
    networth_strength_score = max(0, min((networth / total_assets) * 100, 100)) if total_assets > 0 else 0

    goals = db.query(Goal).filter(Goal.user_id == uid).all()
    if goals:
        goal_progress_values = [(goal.current_amount / goal.target_amount) * 100 for goal in goals if goal.target_amount > 0]
    ...
    final_score = round(savings_rate_score * 0.25 + plan_adherence_score * 0.20 + budget_adherence_score * 0.20 + networth_strength_score * 0.15 + goal_progress_score * 0.20, 2)

    if   final_score >= 80: status = "Strong"
    elif final_score >= 60: status = "Good"
    elif final_score >= 40: status = "Fair"
    else:                   status = "Poor"

    return {
        "month": month, "year": year, "score": final_score, "status": status,
        "components": {
        ...
        "weights": {"savings_rate": 25, "plan_adherence": 20, "budget_adherence": 20, "networth_strength": 15, "goal_progress": 20},
        "budget_summary": {"total_budget": round(total_budget, 2), "total_spent": round(total_budget_spent, 2), "categories": budget_category_results},
        "explanation": {"savings_rate_pct": round(savings_rate_pct, 2), "monthly_income": round(income, 2), "monthly_expenses": round(expenses, 2), "monthly_savings": round(savings, 2), "networth": round(networth, 2)}
    }