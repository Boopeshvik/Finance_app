...
def get_dashboard_overview(
    ...
    yearly_plan = (
        db.query(YearlyPlan)
        .filter(YearlyPlan.user_id == uid, YearlyPlan.year == year)
        .first()
    )
    ...
    for budget in budgets:
        ...
        if   category_spent < budget.amount: status = "within_budget"
        elif category_spent == budget.amount: status = "on_budget"
    ...
    if latest_asset_date:
        total_assets = (
            ...
            .filter(Asset.user_id == uid, Asset.date == latest_asset_date)
            .scalar()
        )
    ...
    if latest_liability_date:
        total_liabilities = (
            ...
            .filter(Liability.user_id == uid, Liability.date == latest_liability_date)
            .scalar()
        )
    ...
    for goal in goals:
        ...
        goal_progress_values.append(progress_pct)
    ...
    if budgets:
        ...
        for budget in budgets:
            ...
            if budget.amount > 0:
                score = 100 if category_spent <= budget.amount else max(0, 100 - ((category_spent - budget.amount) / budget.amount) * 100)
            else:
                score = 0
    ...
    networth_strength_score = max(0, min((networth / total_assets * 100) if total_assets > 0 else 0, 100))
    ...
    if   final_score >= 80: score_status = "Strong"
    elif final_score >= 60: score_status = "Good"
    elif final_score >= 40: score_status = "Fair"
